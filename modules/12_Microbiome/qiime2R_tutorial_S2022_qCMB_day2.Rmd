...
**'Wild type and PD' and 'susceptible and PD' are clustered together on the PC1 axis. Similarly, 'Susceptible and healthy' and 'Wild type and healthy' are clustered together along the same PC1 axis. There is no clear clustering along PC2 axis.**
...
**Jaccard and Unweighted UniFrac are very similar in their beta diversity patterns as the difference between clusters is explained by PC1. Weighted UNiFrac is different from the other two in that the clustering is explained by a combination of PC1 and PC2**
...
**Yes there are alot of differentially abundant taxa. No you cannot tell which groups those taxa are differentially abundant between**
...
S_Healthy_WT_PD <- tab_diff %>%
  select(`S.Healthy - S.PD`,`S.Healthy - WT.PD`,) %>% 
  filter(`S.Healthy - WT.PD` == TRUE)

S_Healthy_WT_Healthy <- tab_diff %>%
  select(`S.Healthy - S.PD`,`S.Healthy - WT.Healthy`,) %>% 
  filter(`S.Healthy - WT.Healthy` == TRUE)

```

Which taxa are differentially abundant between the genotype and donor status groups?

***Collinsella, Paraprevotella, Alistipes, Enterococcus, Christensenella, Pseudoramibacter_Eubacterium, Ruminococcus, Blautia, rc4-4, Anaerofilum, Butyricicoccus, Gemmiger, Holdemania, Sutterella, and Akkermansia are differentially abundant between S.Healthy and WT.PD.***

***Butyricimonas, Paraprevotella, Bacteroides, Parabacteroides, Alistipes, Cloacibacterium, Turicibacter, Christensenella, Pseudoramibacter_Eubacterium, Blautia, Clostridium, Coprococcus, Anaerofilum, Faecalibacterium, Oscillospira, Ruminococcus, and Unknown are differentially abundant between S.Healthy and WT.Healthy.***
...
**Metagenomic sequencing yields whole genomes of all organisms in a sample while amplicon sequencing yields specific barcode sequences for all organisms which contain that specific barcode gene. Metagenome sequencing allows you to infer the genetic potential as well as identiy of the taxa present but is costly. Amplicon sequencing only allows you to infer relative abundances of taxa present but is cheaper**
...
**All the data linked to your samples. Without this metadata, our data analysis would be meaningless.**
...
**Susceptible and PD vs. Susceptible and Healthy: p = 0.00013**

**Wild type and PD vs. Wild type and Healthy: p = 0.00149 **

Day 2 (2 pts each):

6.  What is rarefaction and what are the benefits and drawbacks?

**Rarefaction standardizes the comparison of species richness or diversity across samples with different sample sizes. It allows for fair comparison of diversity measures among samples with different sample sizes but loss of information due to standardization.**

7.  What does beta diversity show that alpha diversity doesn't?

**Beta diversity shows the variation between different samples** 

8.  What is ANCOM-BC used for and why did we use it in this class? 
**It is used to identify taxa that are significantly different between groups while accounting for variability in microbiome data.**
...
**relative abundances between samples**
